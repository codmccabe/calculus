...
\begin{example}
  ...
  \begin{multipleChoice}
    \choice{$H(x)=e^x+\frac{1}{x}$}
    \choice{$H(x)=e^x+\frac{1}{x^2}$}
    \choice[correct]{$H(x)=e^x+\ln{x}$}
  \end{multipleChoice}
  \begin{feedback}
...
\begin{image}
	...
	every axis y label/.style={at=(current axis.above origin),anchor=south},
	every axis x label/.style={at=(current axis.right of origin),anchor=west},]
	\addplot [very thick, penColor2, smooth] {ln(x)};
	\addplot [very thick, penColor2, smooth] {1+ln(x)};
    \addplot [very thick, penColor2, smooth] {3+ln(x)};
    \addplot [very thick, penColor2, smooth] {2+ln(x)};
    \addplot [very thick, penColor, smooth] {-1+ln(x)};
    \addplot [color=penColor,fill=penColor,only marks,mark=*] coordinates{(1,-1)};  %% closed hole
    \node at (axis cs:0,-1.1) [penColor2,fill=white] {$y=\ln(x)$};
    \node at (axis cs:0,-2.2) [penColor,fill=white] {$f(x)=\ln(x)-1$};
    \node at (axis cs:0.2,-0.1) [penColor2,fill=white] {$y=\ln(x)+1$};
    \node at (axis cs:0,0.9) [penColor2,fill=white] {$y=\ln(x)+2$};
    \node at (axis cs:0,1.8) [penColor2,fill=white] {$y=\ln(x)+3$};
    \node at (axis cs:1,-1) [penColor,below right] {$(1,-1)$};
    \end{axis}
	\end{tikzpicture}
\end{image}
...
Since
\[
\ln(1) = \answer[given]{0}\text{ and }f(1)=\answer[given]{0}+C,
\]
 it follows that
\[
-1=\answer[given]{0}+C.
