...
\begin{image}
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot [very thick, penColor, samples=100,smooth, domain=(-6.75:6.75)] {ln(x)/(exp(x-1)-1)};

      \addplot[color=penColor,fill=white,only marks,mark=*] coordinates{(1,1)};  %% open hole
    \end{axis}
  \end{tikzpicture}
\end{image}
...
\begin{question}
  Use the graph of $f(x) = \frac{\ln(x)}{e^{x-1}-1}$ above to finish the following statement: ``A good guess is that\dots''
  \begin{multipleChoice}
    \choice[correct]{$\lim_{x\to 1}\frac{\ln(x)}{e^{x-1}-1} = 1$.}
    \choice{$\lim_{x\to 1}\frac{\ln(x)}{e^{x-1}-1} = 0$.}
    \choice{$\lim_{x\to 1}f(x) = \frac{\ln(1)}{1}$.}
    \choice{$\lim_{x\to 1}f(x) = \frac{\ln(1)}{e^0-1}=\infty$.}
  \end{multipleChoice}
\end{question}
