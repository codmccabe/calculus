...
\begin{question}
  ...
  \begin{prompt}
    \begin{align*}
      \pp[F]{x} &= \answer{3x^2y^2+2y^3}\\
      \pp[F]{y} &= \answer{2x^3y+6xy^2}\\
      \frac{\partial^2F}{\partial x^2} &= \answer{6xy^2}\\
...
\begin{question}
  Let $F(x,y) = x^2e^{\sqrt{2}y}$, compute:
  \[
  \grad F(x,y)
  \begin{prompt}
    = \vector{1,1}
...
instance, if $\vec{p}= \vector{\sqrt{2},\sqrt{2}}$ we compute:
    \[
    \grad F(\vec{p})
    \begin{prompt}
      =\vector{\answer{2\sqrt{2}}\cdot e^2\,,\,\answer{2}\cdot e^2}.
    \end{prompt}
    \]
  \end{question}
\end{question}
