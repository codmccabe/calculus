...
\begin{question}
\author{Nela Lakos}
Which of the following famous functions are increasing on $\Bigl(0,1)\Bigr)$?
\begin{selectAll}
\choice[correct]{$e^x$}
\choice{$e^{-x}$}
\choice{$-\ln(x)$}
...
\end{selectAll}
\begin {explanation} The function $e^{-x}$ is not increasing on $I=(0,1)$, because if we take a pair of numbers in $I$, say $x_1=\frac14$, and $x_2=\frac34$, then $x_1<x_2$, but $f(x_1)>f(x_2)$, since $f(x_1)=e^{-\frac14}\approx 0.7788$, and $f(x_2)=e^{-\frac34}\approx 0.4724$.
 \end{explanation}
\end{question}
