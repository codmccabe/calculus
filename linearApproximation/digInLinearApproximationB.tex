...
Use a linear approximation of $f(x) =\ln(x)$ at $a=1$ to approximate
$\ln(1.3)$.
\begin{explanation}
To start, write
\[
\ddx f(x) = \answer[given]{\frac{1}{x}},
\]
so our linear approximation is
\begin{align*}
L(x) &= 0+\answer[given]{\frac{1}{1}}\cdot(x-1)\\
&= x-1.
\end{align*}
\begin{image}
%\begin{marginfigure}
\begin{tikzpicture}
	\begin{axis}[
            xmin=0,xmax=2.6,ymin=-1,ymax={ln(2.6)+0.5},
            axis lines=center,
            xtick={1,1.3},
            xticklabels={1,1.3},
            ytick={-1,1},
          ...
          \addplot [very thick, penColor, samples=100,smooth, domain=(0:2)] {ln(x)};
          \addplot [domain = (0:2),very thick, penColor2, samples=100,smooth] {x-1};
          \addplot [textColor,dashed] plot coordinates {(1.3,0) (1.3,{ln(1.3)})};
          \node at (axis cs:2.1,.7) [penColor] {$f$};
          \node at (axis cs:2.1,1.1) [penColor2] {$L$};
          \addplot[color=penColor3,fill=penColor3,only marks,mark=*] coordinates{(1,0)};  %% closed hole
            \addplot[color=penColor3,fill=penColor,only marks,mark=*] coordinates{(1.3,0)};  %% closed hole
        \end{axis}
\end{tikzpicture}
%\caption{A linear approximation of $f(x) = \sin(x)$ at $x=0$.}
%\label{figure:la sin}
%\end{marginfigure}
\end{image}
Hence, a linear approximation for $\ln(x)$ at $a=1$ is $L(x) = x-1$,
and so $L(1.3) = 0.3$.  Comparing this to $\ln(.3) \approx 0.262$,
we see that the approximation is quite good. For this reason, it is common
to approximate $\ln(x)$ with its linear approximation $L(x) = x-1$
when $x$ is near one.
...
\begin{example}
Use differentials to approximate $\ln(1.3)$.
\begin{explanation}
Set $y = \ln(x)$. We want to know $\ln(1.3)$. Since $\ln(1) = 0$,
we will set $x = 1$ and $\d x=0.3$. Write with me
\begin{align*}
  \ln(1.3) = \ln(x + \d x) &\approx  \ln(x) + \d y\\
  &\approx 0 + \d y.
  \end{align*}
  Here we see a plot of $y=\ln(x)$ with the differentials above
  marked:
  \begin{image}
%\begin{marginfigure}
\begin{tikzpicture}
	\begin{axis}[
            xmin=0.5,xmax=1.6,ymin=-0.2,ymax=0.55
            2,
            axis lines=center,
            xtick={ 1, 1.3},
            xticklabels={ $1$,$1.3$},
          ...
          \addplot [ thick, penColor, samples=100,smooth, domain=(0:1.8)] {ln(x)};
          %\addplot [penColor2,very thick] plot coordinates {(0,0) (.3,0)};
          %\addplot [penColor2,very thick] plot coordinates {(.3,0) (.3,.3)};
          \addplot [draw=black,dashed,->,>=stealth'] plot coordinates {(1,0) (1.3, 0)};
          \addplot [draw=penColor2,->,>=stealth'] plot coordinates {(1.3,0) (1.3, 0.3)};
            \addplot [ thick, penColor2,smooth] {x-1};
          \node at (axis cs:1.45,0.26) [penColor] {\tiny$y=\ln(x)$};
           \node at (axis cs:1.34,0.46) [penColor2] {\tiny$y=L(x)=x$};
          \node [below] at (axis cs:1.15,0) {$\d x$};
          \node [right] at (axis cs:1.3,0.15) [penColor2]{$\d y$};
          \node [above left, color=penColor3] at (axis cs:1,0) {$(1,0)$};
          \addplot[color=penColor3,fill=penColor3,only marks,mark=*] coordinates{(1,0)};  %% closed hole
        \end{axis}
\end{tikzpicture}
...
\begin{align*}
  \d y &= \left(\ddx \ln(x)\right) \cdot \d x\\
  &=\answer[given]{\frac{1}{1}} \cdot \d x\\
  &= 1 \cdot (\answer[given]{0.3})\\
  &= 0.3
\end{align*}
Hence $\ln(1.3) \approx \ln(0) + 0.3 \approx 0.3$.
\end{explanation}
\end{example}
